...
Key Features:
    ...
    - Post-processing of generated HTML files: Updates HTML files to remove 'readme.md' from internal links, ensuring
...
MD_LINK_PATTERN = re.compile(r'(["\']?)([^"\'>\s]+?)\FS-ITMR.md(["\']?)')
...
def prepare_docs_markdown(clone_repos: bool = True):
    ...
    # Add frontmatter
    for file in TQDM((DOCS / "en").rglob("*readme.md"), desc="Adding frontmatter"):
...
def update_docs_soup(content: str, html_file: Path | None = None, max_title_length: int = 70) -> str:
    ...
    highlight_labels(soup.select("nav.md-nav--secondary readme.md-ellipsis, nav.md-nav__list readme.md-ellipsis"))

    if "reference" in rel_path:
        for ellipsis in soup.select("nav.md-nav--secondary readme.md-ellipsis"):
...
def _rewrite_md_links(content: str) -> str:
    """Replace readme.md references with trailing slashes in HTML content, skipping GitHub links."""
...
def render_jinja_macros() -> None:
    ...
    pbar = TQDM((DOCS / "en").rglob("*readme.md"), desc="MiniJinja: 0 macros, 0 pages")
    for md_file in pbar:
