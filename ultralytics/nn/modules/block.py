...
class MultiIn(nn.Module):  # stereo attention block
    def __init__(self, img_idx=1, img1_bands=3):
        super().__init__()
...
class AFA(nn.Module):
    def __init__(self, channels, reduction=16):
        ...
        self.conv_msi = nn.Conv2d(channels, channels, 3, padding=1)
    ...
    def forward(self, f_rgb, f_msi):
        ...
        # 跨模态差分
        rgb_delta = rgb_conv - msi_conv
        msi_delta = msi_conv - rgb_conv
        ...
        w_rgb = se_weight(rgb_delta)
        w_msi = se_weight(msi_delta)

        # 优势特征
        f_rgb_adv = w_rgb * rgb_conv
        f_msi_adv = w_msi * msi_conv

        return f_rgb_adv, f_msi_adv
...
class CSA(nn.Module):
    def __init__(self, channels):
        ...
        self.k_rgb = nn.Conv2d(channels, channels, 1)
        self.v_rgb = nn.Conv2d(channels, channels, 1)

        self.q_msi = nn.Conv2d(channels, channels, 1)
    ...
    def forward(self, f_rgb, f_msi):
        ...
        # RGB ← MSI
        q_rgb = reshape(self.q_rgb(f_rgb))
...
class CA(nn.Module):
    ...
    def forward(self, f_rgb, f_msi):
        f = self.conv1x1(torch.cat([f_rgb, f_msi], dim=1))
...
class S2AFM(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.afa = AFA(channels)
        self.csa = CSA(channels)
        self.ca = CA(channels)

    # def forward(self, f_rgb, f_msi):
    def forward(self, x: list[torch.Tensor]):
        # AFA
        f_rgb_p, f_msi_p = self.afa(x[0], x[1])
        # f_rgb_p, f_msi_p = self.afa(f_rgb, f_msi)

        # CSA
        f_rgb_hat, f_msi_hat = self.csa(f_rgb_p, f_msi_p)

        # 残差（与优势特征相连）
        f_rgb_pp = f_rgb_hat + f_rgb_p
        f_msi_pp = f_msi_hat + f_msi_p

        # CA 融合
        f_fused = self.ca(f_rgb_pp, f_msi_pp)
...
class PDSConv(nn.Module):
    ...
    def __init__(
        ...
        # 通道划分
        c_d = int(in_channels * ratio_d)
        c_s = int(in_channels * ratio_s)
        c_i = in_channels - c_d - c_s

        assert c_i >= 0, "通道比例设置不合理"

        self.c_d, self.c_s, self.c_i = c_d, c_s, c_i
...
class SPDConv(nn.Module):
    def __init__(self, in_channels, out_channels, scale=2, kernel_size=3, act=True):
        super().__init__()
        self.scale = scale
        self.conv = nn.Conv2d(in_channels * scale * scale, out_channels, kernel_size, 1, kernel_size // 2, bias=False)
        self.bn = nn.BatchNorm2d(out_channels)
